...
| 17 | NationalityClaim | **circuit done (Day 22)** — `nationality.circom` on the shared base, `requiredNationality === nationality` in the generic slot, nine-signal layout, constraints locked, `--inspect` clean, 14 tests green (wrong nationality / tampered field / wrong issuer / expired all rejected). Setup, generated verifier, frozen layout and `submitNationalityClaim` remain (Day 23); deploy and index (Day 24) |
